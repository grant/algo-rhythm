...
class AlgorythmDatamodel:
  ...
  def startTrainingProcess(self, newProcessName, targetConfig, xmlFileList, numIterations, startConfig = None):
    ...
    cmd = ['python', 'train_dummy.py', AlgorythmDatamodel.CONFIG_DIR + targetConfig, ' '.join([AlgorythmDatamodel.UPLOAD_DIR + xmlfile for xmlfile in xmlFileList]), str(numIterations)]
  ...
  def startMusicGenerationProcess(self, newProcessName, trainedConfig, outputSongName, numSecondsToGenerate):
    ...
    if outputSongName in self.getMusicBeingGenerated():
      raise ValueError("target music file  {} is already being generated by currently running process".format(outputSongName))

    cmd = ['python', 'genmusic_dummy.py', AlgorythmDatamodel.CONFIG_DIR + trainedConfig, AlgorythmDatamodel.GENERATED_SONG_DIR + outputSongName, str(numSecondsToGenerate)]
    processTriple = self.__startProcessFromCommand(cmd)
