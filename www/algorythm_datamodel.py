...
class AlgorythmDatamodel:
    # some constants:
    #CHANGE: Now these are fields set by constructor,
    #probably would not be terrible to lowercase these
    #UPLOAD_DIR = 'backend/music_uploads/'
    #CONFIG_DIR = 'backend/trained_configs/'
    #GENERATED_SONG_DIR = 'backend/generated_music/'
    #SCRIPT_ROOT = '../dummycode/'
    ...
    def __init__(self, basedir):
        ...
        self.SCRIPT_ROOT = basedir + '/'
        self.UPLOAD_DIR = basedir + '/training_xml_web/'
        self.CONFIG_DIR = basedir + '/trained_configs/'
        self.GENERATED_SONG_DIR = basedir + '/generated_music/'
    ...
    def getTrainingFiles(self):
        self.__cleanup()
        return self.__getFilesInDir(self.UPLOAD_DIR)

    def getCompletedTrainedConfigs(self):
        self.__cleanup()
        return self.__getFilesInDir(self.CONFIG_DIR)

    def getGeneratedMusic(self):
        self.__cleanup()
        return self.__getFilesInDir(self.GENERATED_SONG_DIR)
    ...
    def startTrainingProcess(self, newProcessName, targetConfig, xmlFileList, numIterations, startConfig=None):
        ...
        cmd = ['python', self.SCRIPT_ROOT + 'train.py', self.CONFIG_DIR + targetConfig,
               ' '.join([self.UPLOAD_DIR + xmlfile for xmlfile in xmlFileList]), str(numIterations)]
    ...
    def startMusicGenerationProcess(self, newProcessName, trainedConfig, outputSongName, numSecondsToGenerate):
        ...
        cmd = ['python', self.SCRIPT_ROOT + 'genmusic.py', self.CONFIG_DIR + trainedConfig,
               self.GENERATED_SONG_DIR + outputSongName, str(numSecondsToGenerate)]
        processTriple = self.__startProcessFromCommand(cmd)
...
if __name__ == '__main__':
    adm = AlgorythmDatamodel('../dummycode')
